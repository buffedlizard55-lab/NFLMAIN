...
| Test suite | 121 tests (as of 2026-09-25) covering null discipline, status derivation, link construction, aggregation, report/index honesty, workflow-file validity, the direct nfl.com reader, the identifier correction, same-week scoping, partial-audit honesty, the browser-global guard, and a full offline build |
| Frontend executed in CI | `tests/frontend_smoke.js` runs the site's real JS against a real build through a DOM shim and fails if `undefined` / `NaN` / `[object Object]` reaches the screen, or if a page renders empty. The shim shadows Node's `global` with `undefined` so Node-only identifiers (the bug that once left every page blank) throw in CI exactly as they do in a browser |
...
| Blank-site outage found and fixed (2026-09-25, session 4) | `scoreboard.js`, `history.js` and `game.js` referenced the Node-only identifier `global` (6 lines). Every browser threw `ReferenceError` at `DOMContentLoaded` and the pages stayed on "Loading feed..." — no scoreboard, no archive, no game detail; only the Sources page (which never used `global`) worked. Root cause: the DOM shim executed site scripts inside Node's global scope, where `global` exists, so 120 green tests shipped a dead site. Fixed by switching to `window`, shadowing `global` in the harness, and adding a static guard test. Verified by rendering all four pages in jsdom against the deployed bytes (zero page errors, real game cards) and by 121 passing tests |
...
| 2026-09-25 (session 4) | site rendered blank | Owner reported "there's nothing, no scoreboard". Confirmed and root-caused: `global.location` in `scoreboard.js:30` (plus 5 more lines in `history.js`/`game.js`) is a Node-ism — browsers have no `global`, so `boot()` threw before first paint. The test harness masked it by executing scripts in Node's global scope | Reproduced with the harness hardened (`global` shadowed → 13 failures, "page body is nearly empty"), fixed, then verified: jsdom render of all four deployed pages shows real game cards (ATL 35 GB 14 with quarter lines) and zero page errors; 121 tests + frontend smoke green; new static guard test proven to fail on the buggy line |
